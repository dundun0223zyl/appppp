        ...
        if st.button("🔮 Get Prediction"):
            if selected_ticker:
                ...
                if pred_response.status_code == 200 and hist_response.status_code == 200:
                    ...
                    # Ensure marker appears clearly at the transition point
                    #transition_point = datetime.today().strftime("%Y-%m-%d")  # Ensure correct transition point
                    ...
                    chart_series = [
                        ...
                        #{"type": "Marker", "data": [{"time": transition_point, "position": "aboveBar", "color": "red", "shape": "arrowUp", "text": "**Prediction Start**", "size": 12}]},  # Red Marker
                    ]
                    # Render the chart
            ...
            st.warning("⚠ Please enter a valid ticker before proceeding.")
                    ...
                    #hist_response = requests.get(f"{API_BASE_URL}/stock/{selected_ticker}")
                    #hist_prices = hist_response.json().get("historical_prices", []) if hist_response.status_code == 200 else []

                    #pred_response = requests.get(f"{API_BASE_URL}/predict/{selected_ticker}?days_ahead={days_ahead}")
                    #pred_data = pred_response.json() if pred_response.status_code == 200 else {}

                    #if hist_prices and pred_data:
                        # Ensure Historical Data is Properly Filtered
                        #historical_dates = []
                        #historical_prices = []

                        #for entry in hist_prices:
                            #date_obj = datetime.strptime(entry["date"], "%Y-%m-%d")
                            #if date_obj >= start_date:
                                #historical_dates.append(date_obj)
                                #historical_prices.append(entry["close"])

                        # Ensure Future Dates are Correct
                        #future_dates = [(end_date + timedelta(days=i)) for i in range(1, len(pred_data["predictions"]) + 1)]
                        #future_prices = pred_data["predictions"]
                        ...
                        # Format Layout
                        #fig.update_layout(
                            #title=f"{selected_ticker.upper()} Stock Price Forecast",
                            #xaxis_title="Date",
                    ...
                    #else:
                        #st.error("❌ Failed to fetch historical or prediction data. Please try again.")
